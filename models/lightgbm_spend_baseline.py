...
# NO PARAMS - uses LightGBM library defaults
...
def train(df: pd.DataFrame) -> tuple:
    ...
    print(f"LightGBM Spend (Baseline) -- RMSE: {rmse:.4f}  MAE: {mae:.4f}")
    return model, encoders, {"rmse": rmse, "mae": mae}


def save(model, encoders, path: str = "models/lgbm_spend_baseline.pkl"):
